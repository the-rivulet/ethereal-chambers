...
function generateLanguage() {
  ...
  for(let i of ["question", "pronoun", "ownership", "indicator", "is", "not", "can", "know", "go", "and", "1", "2", "3", "4", "5"]) {
    ...
    while(!candidate || Object.values(l.rootList).includes(candidate)) candidate = randomSyllable() + (Math.random() < 0.3 ? randomSyllable() : "");
  ...
  for(let i of [
    ...
    while(!candidate || Object.values(l.rootList).includes(candidate)) {
      let rootWord = (Math.random() < l.useRootChance && i.length > 1) ? (i.length >= 3 ? l.rootList[i[1]] + l.rootList[i[2]] : l.rootList[i[1]]) : randomSyllable();
      let suffix = (Math.random() < 1 - 0.3 * i.length ? randomSyllable() : "") + (Math.random() < 0.2 ? randomSyllable() : "");
...
beginButton.onclick = function() {
  ...
  let scene1 = new Scene(awakenChamber, `You find yourself in a room you've never seen before. The sound of a door slamming shut echoes in your mind, but you can't tell from where.`, [
    {name: "Look around", click() {
      new Scene(awakenChamber, () => `This area seems pretty barren. As you look around, someone comes into the room and waves to you. 'a, ${toki ? "sina lape ala. nimi sina li seme" : l.f_translate("you", "awaken") + ". " + l.f_translate("you", "is", "who")}?'<br/>They look like they're barely staying awake...`, [
        {name: "'huh what?'", click() {
          new Scene(awakenChamber, () => `The ${toki ? "soweliko" : l.f_translate("cat")} sighs...<br/>'... ${toki ? "mi ken pana e sona pona tawa sina" : l.f_translate("i", "can teach", "you") + ". " + l.f_translate("", "come")}.'`, [
      ...
      let introductions = new Scene("Chamber I", () => `'${toki ? "mi jan Wisu" : l.f_translate("i", "is", "Rivu")}.' They pause for a moment. '${toki ? "sina jan seme" : l.f_translate("who", "is", "you")}?'`, [
        {name: "'... " + (toki ? "jan Suwiko" : l.translate("Survi")) + "?'", click() {
          new Scene("Chamber I", () => `'${toki ? "ni li nimi sina" : l.f_translate("that", "is", "your name")}? a... ${toki ? "jan Wisu en jan Suwiko" : ["Rivu", "and", "Survi"].map(x => l.f_translate(x)).join(" ")}.'<br/>
          ${toki ? "jan Wisu" : l.f_translate("Rivu")} points at the door you just came through. '${toki ? "ni li tomo sina" : l.f_translate("this", "is", "your awaken room")}.'`, [
            {name: l.translate("my awaken room") + "?", click() {
              new Scene("Chamber I", () => `'a.'<br/>${toki ? "jan Wisu" : l.f_translate("Rivu")} looks pretty tired... '${toki ? "mi kama lape" : l.f_translate("i", "go sleep") + ". " + l.f_translate("sleep") + "- " + l.f_translate("not awaken")}.'`).load();
            }}
          ]).load();
        }},
        {name: "Stay silent", click() {
    ...
    {name: "Try to exit the room", click() {
      new Scene(awakenChamber, () => `The door clangs. You hear the sound of a code being entered on the other side... you feel like you might have attracted some attention.`, [
        {name: "Bang on the door more", click() {
          new Scene(awakenChamber, () => `You hear a voice from the other side of the door. '${toki ? "sina ken ala awen...?" : l.f_translate("you", "is", "not patient") + "..."}'`).load();
        }}
      ]).load();
    }}
  ]);
